...
// MapLibre paint takes a colour string and has no access to a token; and two
// files of that same kind that happen to sit elsewhere. `darkMatterStyle.ts` is
// a third-party basemap style, CartoDB Dark Matter adapted for OpenFreeMap, whose
// halos and casings are cartography rather than app colour; naming them after app
// tokens would claim a relationship that is not there. `maplibreRenderer.generated.ts`
// is the vendored MapLibre GL JS bundle, regenerated by a script and marked "do
// not edit".
//
// The test is whether a token exists for the colour, not whether the value ends
