...
export const OWNED_ELSEWHERE: Record<string, string> = {
  ...
  // Dead, not owned elsewhere. Its own doc comment names the synthetic
  // detection illustration in settings as its only caller, and that component
  // was deleted. The Rust export goes with it, but removing a uniffi export
  // needs the bindings regenerated by a native build: the generated binding
  // asserts this function's own checksum at engine init, so deleting the Rust
  // side alone gives an engine that refuses to start.
  detectSectionsStandalone:
    'dead since the detection illustration was deleted, and removing the Rust export needs the bindings regenerated by a native build',
