...
/// Get the list of tile coordinates that need to be generated for all stored activities.
///
/// Returns a list of (zoom, x, y) tuples representing tiles that contain GPS data.
/// Use this to show progress or generate tiles incrementally.
#[uniffi::export]
pub fn ffi_get_tile_coords(config: FfiTileConfig) -> Vec<Vec<u32>> {
    init_logging();
    let ffi_start = Instant::now();
    info!("[RUST: ffi_get_tile_coords] Starting tile coordinate calculation");

    // Get all GPS tracks from persistent engine
    let tracks = crate::persistence::with_persistent_engine(|engine| {
        engine.get_all_tracks()
    }).unwrap_or_default();

    info!(
        "[RUST: ffi_get_tile_coords] Retrieved {} tracks from engine",
        tracks.len()
    );
    ...
    let tile_coords = crate::tiles::tiles_for_tracks(&tracks, config.min_zoom, config.max_zoom);

    // Convert to FFI-friendly format: [[z, x, y], ...]
    let result: Vec<Vec<u32>> = tile_coords
        .into_iter()
        .map(|(z, x, y)| vec![z as u32, x, y])
        .collect();

    info!(
        "[RUST: ffi_get_tile_coords] Found {} tiles to generate ({} ms)",
        result.len(),
        elapsed_ms(ffi_start)
    );
...
/// Generate all heatmap tiles for stored activities.
///
/// This loads all GPS tracks from the persistent engine and generates PNG tiles
/// at the configured zoom levels. Returns the generated tiles as PNG byte arrays.
///
/// For large datasets, consider using ffi_generate_tiles_for_coords() to generate
/// tiles in batches and show progress.
#[uniffi::export]
pub fn ffi_generate_heatmap_tiles(config: FfiTileConfig) -> Vec<FfiTileResult> {
    init_logging();
    let ffi_start = Instant::now();
    info!("[RUST: ffi_generate_heatmap_tiles] Starting tile generation");
    ...
    info!(
        "[RUST: ffi_generate_heatmap_tiles] Retrieved {} tracks ({} ms)",
        tracks.len(),
        elapsed_ms(tracks_start)
    );
    ...
    let internal_config: crate::tiles::TileConfig = config.into();

    // Generate tiles
    let generate_start = Instant::now();
    let tiles = crate::tiles::generate_all_tiles(&tracks, &internal_config);

    info!(
        "[RUST: ffi_generate_heatmap_tiles] Generated {} tiles ({} ms)",
        tiles.len(),
        elapsed_ms(generate_start)
    );

    // Convert to FFI format
    let result: Vec<FfiTileResult> = tiles.into_iter().map(Into::into).collect();

    info!(
        "[RUST: ffi_generate_heatmap_tiles] Complete ({} ms total)",
        elapsed_ms(ffi_start)
    );
...
///
/// Use this with ffi_get_tile_coords() to generate tiles in batches.
/// Pass a list of [z, x, y] coordinates to generate.
#[uniffi::export]
pub fn ffi_generate_tiles_for_coords(
    coords: Vec<Vec<u32>>,
    config: FfiTileConfig,
) -> Vec<FfiTileResult> {
    init_logging();
    let ffi_start = Instant::now();
    info!(
        "[RUST: ffi_generate_tiles_for_coords] Generating {} tiles",
        coords.len()
    );
    ...
    // Generate each requested tile
    let results: Vec<FfiTileResult> = coords
        .into_iter()
        .filter_map(|c| {
            if c.len() >= 3 {
                let z = c[0] as u8;
                let x = c[1];
                let y = c[2];
                let tile = crate::tiles::generate_tile(z, x, y, &tracks, &internal_config);
                Some(tile.into())
            } else {
                None
            }
    ...
    info!(
        "[RUST: ffi_generate_tiles_for_coords] Generated {} tiles ({} ms)",
        results.len(),
        elapsed_ms(ffi_start)
    );
...
/// Save tiles to the filesystem.
///
/// Tiles are saved in the standard z/x/y.png format under the given base path.
/// Returns the number of tiles successfully saved.
#[uniffi::export]
pub fn ffi_save_tiles(tiles: Vec<FfiTileResult>, base_path: String) -> u32 {
    init_logging();
    let ffi_start = Instant::now();
    info!(
        "[RUST: ffi_save_tiles] Saving {} tiles to {}",
        tiles.len(),
        base_path
    );
    ...
    if let Err(e) = std::fs::create_dir_all(path) {
        info!("[RUST: ffi_save_tiles] Failed to create directory: {}", e);
        return 0;
    }

    let mut saved = 0u32;
    for tile in tiles {
        // Convert back to internal type for saving
        let internal_tile = crate::tiles::TileResult {
            png_data: tile.png_data,
            z: tile.z,
            x: tile.x,
            y: tile.y,
        };

        if crate::tiles::save_tile(&internal_tile, path).is_ok() {
            saved += 1;
        }
    }

    info!(
        "[RUST: ffi_save_tiles] Saved {}/{} tiles ({} ms)",
        saved,
        saved,
        elapsed_ms(ffi_start)
    );
...
/// Generate and save heatmap tiles in one operation.
///
/// This is more efficient than generating and saving separately as it avoids
/// transferring tile data across the FFI boundary.
///
/// Returns a result with statistics about the operation.
#[derive(Debug, Clone, uniffi::Record)]
pub struct TileGenerationResult {
    /// Number of tiles generated
    pub tiles_generated: u32,
    /// Number of tiles saved to disk
    pub tiles_saved: u32,
    /// Total generation time in milliseconds
    pub generation_time_ms: u32,
    /// Total save time in milliseconds
    pub save_time_ms: u32,
    /// Number of GPS tracks processed
    pub tracks_processed: u32,
}

#[uniffi::export]
pub fn ffi_generate_and_save_tiles(
    base_path: String,
    config: FfiTileConfig,
    ...
    let ffi_start = Instant::now();
    info!(
        "[RUST: ffi_generate_and_save_tiles] Starting tile generation to {}",
        base_path
    );
    info!(
        "[RUST: ffi_generate_and_save_tiles] Config: zoom {}-{}, color rgba({},{},{},{}), width {}",
        config.min_zoom, config.max_zoom,
        config.line_color_r, config.line_color_g, config.line_color_b, config.line_color_a,
        config.line_width
    );
    ...
    let tracks_count = tracks.len() as u32;
    let total_points: usize = tracks.iter().map(|t| t.len()).sum();

    info!(
        "[RUST: ffi_generate_and_save_tiles] Retrieved {} tracks with {} total points ({} ms)",
        tracks_count,
        total_points,
        elapsed_ms(tracks_start)
    );

    // Debug: sample the first track's first few points
    if let Some(first_track) = tracks.first() {
        let sample: Vec<_> = first_track.iter().take(5).collect();
        info!(
            "[RUST: ffi_generate_and_save_tiles] First track has {} points. Sample: {:?}",
            first_track.len(),
            sample
        );
        // Check validity
        let valid_count = first_track.iter().filter(|p| p.is_valid()).count();
        info!(
            "[RUST: ffi_generate_and_save_tiles] First track: {} valid, {} invalid",
            valid_count,
            first_track.len() - valid_count
        );
    }
    ...
    if tracks.is_empty() {
        return TileGenerationResult {
            tiles_generated: 0,
            tiles_saved: 0,
            generation_time_ms: elapsed_ms(ffi_start) as u32,
            save_time_ms: 0,
            tracks_processed: 0,
        };
    }
    ...
    let internal_config: crate::tiles::TileConfig = config.into();
    info!(
        "[RUST: ffi_generate_and_save_tiles] Calling generate_all_tiles with {} tracks",
        tracks.len()
    );
    let tiles = crate::tiles::generate_all_tiles(&tracks, &internal_config);
    let generation_time = elapsed_ms(generate_start) as u32;
    let tiles_generated = tiles.len() as u32;

    info!(
        "[RUST: ffi_generate_and_save_tiles] Generated {} tiles ({} ms)",
        tiles_generated,
        generation_time
    );
    ...
    let path = std::path::Path::new(&base_path);
    let tiles_saved = crate::tiles::save_all_tiles(&tiles, path).unwrap_or(0) as u32;
    let save_time = elapsed_ms(save_start) as u32;

    info!(
        "[RUST: ffi_generate_and_save_tiles] Saved {} tiles ({} ms)",
        tiles_saved,
        save_time
    );

    info!(
        "[RUST: ffi_generate_and_save_tiles] Complete ({} ms total)",
        elapsed_ms(ffi_start)
    );
