/**
 * Muscle polygons with Bezier sampling and sub-path splitting.
 * Priority-ordered: specific muscles checked before broad ones.
 */
...
/**
 * Find which muscle contains the given point.
 * Uses point-in-polygon against actual SVG shapes first,
 * then falls back to nearest center point if no polygon match.
 */
export function findMuscleAtPoint(
  ...
  // 1. Try exact polygon hit test (priority ordered)
  ...
  // 2. Fallback: nearest center point within threshold
  const positions = side === 'front' ? FRONT_POSITIONS : BACK_POSITIONS;
  const THRESHOLD = 0.08; // ~8% of SVG size
