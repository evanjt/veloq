...
export function computeInsightsFromData(
  ...
  try {
    ...
    const strengthSeries = ffiData.hasStrengthData
      ? ffiData.strengthSeries
      : undefined;
    ...
    const coreInsights = generateInsights(
      {
        ...
        efficiencyTrends,
        sectionChanges,
        strengthMonthly: strengthSeries
          ? normalizeStrengthSummary(strengthSeries.monthly)
          : null,
        strengthWeekly:
          strengthSeries?.weekly.map(normalizeStrengthSummary) ?? [],
      },
      t,
    );
