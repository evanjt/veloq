...
function createDefaultPreferences(enabledIds: MetricId[]): MetricPreference[] {
  return AVAILABLE_METRICS.map((metric, index) => ({
    ...
    order: enabledIds.includes(metric.id)
      ? enabledIds.indexOf(metric.id)
      : index + 100, // Disabled metrics at end
  }));
}
...
export const useDashboardPreferences = create<DashboardPreferencesState>(
  (set, get) => ({
    ...
    resetToDefaults: (sport) => {
      const defaultIds =
        DEFAULT_METRICS_BY_SPORT[sport] || DEFAULT_METRICS_BY_SPORT.Other;
...
function parseStoredSummaryCard(raw: string): SummaryCardPreferences | null {
  ...
  if (
    ...
    typeof merged.showSparkline !== "boolean"
  ) {
    return null;
  }
  ...
  if (
    !Array.isArray(merged.supportingMetrics) ||
    !merged.supportingMetrics.every(isMetricId)
  ) {
    return null;
  }
  return merged;
...
async function persistSummaryCard(
  summaryCard: SummaryCardPreferences,
): Promise<void> {
  try {
    ...
    if (__DEV__) {
      console.warn(
        "[DashboardPreferences] Failed to persist summary card:",
        error,
      );
    }
  }
...
export async function initializeDashboardPreferences(
  ...
  try {
    ...
    const defaultIds =
      DEFAULT_METRICS_BY_SPORT[primarySport] || DEFAULT_METRICS_BY_SPORT.Other;
    const metrics =
...
export function getMetricsForSport(
  metrics: MetricPreference[],
  sport: string,
): MetricPreference[] {
