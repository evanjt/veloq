import * as FileSystem from 'expo-file-system/legacy';
import AsyncStorage from '@react-native-async-storage/async-storage';
import { debug } from '@/shared/debug/debug';
import type { UploadQueueEntry } from '@/types';

const log = debug.create('UploadQueue');

const UPLOADS_DIR = `${FileSystem.documentDirectory}pending_uploads/`;
const QUEUE_KEY = 'veloq-upload-queue';

async function ensureUploadsDir(): Promise<void> {
  const dirInfo = await FileSystem.getInfoAsync(UPLOADS_DIR);
  if (!dirInfo.exists) {
    await FileSystem.makeDirectoryAsync(UPLOADS_DIR, { intermediates: true });
    log.log('Created pending uploads directory');
  }
}

async function loadQueue(): Promise<UploadQueueEntry[]> {
  try {
    const stored = await AsyncStorage.getItem(QUEUE_KEY);
    if (!stored) return [];
    return JSON.parse(stored) as UploadQueueEntry[];
  } catch {
    return [];
  }
}

async function saveQueue(queue: UploadQueueEntry[]): Promise<void> {
  await AsyncStorage.setItem(QUEUE_KEY, JSON.stringify(queue));
}

// Serialize every queue access through one promise chain. The queue is a
// load-modify-save over a single AsyncStorage key, so concurrent callers (the
// upload processor draining while a freshly-saved recording enqueues) would
// otherwise read the same snapshot and the later write would clobber the
// earlier one — silently dropping a queued upload that has no server backstop.
let queueLock: Promise<unknown> = Promise.resolve();
function withQueueLock<T>(fn: () => Promise<T>): Promise<T> {
  const run = queueLock.then(fn, fn);
  queueLock = run.then(
    () => undefined,
    () => undefined
  );
  return run;
}

export async function enqueueUpload(
  entry: Omit<UploadQueueEntry, 'id' | 'retryCount'>
): Promise<string> {
  await ensureUploadsDir();
  const id = `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
  const fullEntry: UploadQueueEntry = { ...entry, id, retryCount: 0 };
  return withQueueLock(async () => {
    const queue = await loadQueue();
    queue.push(fullEntry);
    await saveQueue(queue);
    log.log(`Enqueued upload: ${id}`);
    return id;
  });
}

export function dequeueUpload(): Promise<UploadQueueEntry | null> {
  return withQueueLock(async () => {
    const queue = await loadQueue();
    // Skip permission-blocked entries — they wait for OAuth upgrade
    return queue.find((e) => !e.permissionBlocked) ?? null;
  });
}

export function markUploadComplete(id: string): Promise<void> {
  return withQueueLock(async () => {
    const queue = await loadQueue();
    const entry = queue.find((e) => e.id === id);
    const updated = queue.filter((e) => e.id !== id);
    await saveQueue(updated);

    // Clean up the file
    if (entry) {
      try {
        const info = await FileSystem.getInfoAsync(entry.filePath);
        if (info.exists) {
          await FileSystem.deleteAsync(entry.filePath, { idempotent: true });
        }
      } catch {
        // Best effort cleanup
      }
    }

    log.log(`Upload complete: ${id}`);
  });
}

const MAX_RETRIES = 5;

export function markUploadFailed(id: string, error: string): Promise<void> {
  return withQueueLock(async () => {
    const queue = await loadQueue();
    const entry = queue.find((e) => e.id === id);

    // Remove entries that have exceeded max retries
    if (entry && entry.retryCount + 1 >= MAX_RETRIES) {
      log.warn(`Upload ${id} exceeded ${MAX_RETRIES} retries, removing from queue`);
      const filtered = queue.filter((e) => e.id !== id);
      await saveQueue(filtered);
      // Clean up the file
      try {
        const info = await FileSystem.getInfoAsync(entry.filePath);
        if (info.exists) {
          await FileSystem.deleteAsync(entry.filePath, { idempotent: true });
        }
      } catch {
        // Best effort cleanup
      }
      return;
    }

    const updated = queue.map((e) =>
      e.id === id ? { ...e, retryCount: e.retryCount + 1, lastError: error } : e
    );
    await saveQueue(updated);
    log.log(
      `Upload failed: ${id} (${error}), retry ${(entry?.retryCount ?? 0) + 1}/${MAX_RETRIES}`
    );
  });
}

export function getQueueSize(): Promise<number> {
  return withQueueLock(async () => {
    const queue = await loadQueue();
    return queue.length;
  });
}

export function markUploadPermissionBlocked(id: string): Promise<void> {
  return withQueueLock(async () => {
    const queue = await loadQueue();
    const updated = queue.map((e) => (e.id === id ? { ...e, permissionBlocked: true } : e));
    await saveQueue(updated);
    log.log(`Upload permission-blocked: ${id}`);
  });
}

export function clearPermissionBlocked(): Promise<void> {
  return withQueueLock(async () => {
    const queue = await loadQueue();
    const updated = queue.map((e) =>
      e.permissionBlocked ? { ...e, permissionBlocked: false } : e
    );
    await saveQueue(updated);
    log.log('Cleared permission-blocked flag on all entries');
  });
}

export function getPermissionBlockedCount(): Promise<number> {
  return withQueueLock(async () => {
    const queue = await loadQueue();
    return queue.filter((e) => e.permissionBlocked).length;
  });
}

export function clearUploadQueue(): Promise<void> {
  return withQueueLock(async () => {
    await AsyncStorage.removeItem(QUEUE_KEY);
    try {
      const dirInfo = await FileSystem.getInfoAsync(UPLOADS_DIR);
      if (dirInfo.exists) {
        await FileSystem.deleteAsync(UPLOADS_DIR, { idempotent: true });
        log.log('Cleared upload queue and files');
      }
    } catch {
      // Best effort cleanup
    }
  });
}
