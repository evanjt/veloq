 ...
 * - Red/Orange (#FC4C02): High intensity (very hot, fatigued)
...
export function useActivityStats({
  ...
  const { avgLoad, avgIntensity, avgHR } = useMemo(() => {
    ...
    return {
      avgLoad:
        sameType.reduce((sum, a) => sum + (a.icu_training_load || 0), 0) /
        sameType.length,
      avgIntensity:
        sameType.reduce((sum, a) => sum + (a.icu_intensity || 0), 0) /
        sameType.length,
      avgHR:
        sameType.reduce(
          (sum, a) => sum + (a.average_heartrate || a.icu_average_hr || 0),
          0,
        ) / sameType.length,
    };
  ...
  const stats = useMemo(() => {
    ...
    if (avgHRValue) {
      ...
      const athleteMaxHR =
        activity.icu_hr_zones?.[activity.icu_hr_zones.length - 1] || 200;
    ...
    if (activity.calories && activity.calories > 0) {
      const calPerHour = Math.round(
        (activity.calories / activity.moving_time) * 3600,
      );
      result.push({
