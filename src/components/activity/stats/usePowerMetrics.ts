...
export function usePowerMetrics({
  ...
  const stat = useMemo(() => {
    ...
    const comparison = {
      ...
      value: `${ftpPercent}%`,
      trend: ftpPercent > 75 ? ('up' as const) : ftpPercent < 50 ? ('down' as const) : ('same' as const),
    ...
    if (activity.weighted_average_watts && activity.weighted_average_watts !== activity.average_watts) {
      details.push({
