...
describe("rules.applyMixAndCap (D9, D10)", () => {
  ...
  it("enforces total surface cap when categories leave room", () => {
    ...
    expect(
      dropped.some(
        (d) => d.reason === "surface_cap" || d.reason === "category_cap",
      ),
    ).toBe(true);
  });
  ...
  it("per-category cap bounds the total when few categories exist", () => {
    ...
    const scored = Array.from({ length: 20 }, (_, i) =>
      mk(
        `i${i}`,
        (["hrv_trend", "fitness_milestone", "period_comparison"] as const)[
          i % 3
        ],
        100 - i,
      ),
...
describe("section trend recency (G1 applied to section_trend)", () => {
  ...
  it("keeps a recently-visited old section (event recency, not section age)", () => {
    ...
    const result = generateSectionTrendInsights(
      [freshEffort],
      new Set(),
      NOW,
      mockT,
  ...
  it("boundary: exactly at activeWindowDays passes, 1 day beyond rejects", () => {
    ...
    expect(
      generateSectionTrendInsights([atBoundary], new Set(), NOW, mockT),
    ).toHaveLength(1);
    expect(
      generateSectionTrendInsights([justBeyond], new Set(), NOW, mockT),
    ).toHaveLength(0);
  });
...
describe("ML ranking breakdown on section_trend insights", () => {
  ...
  it("surfaces the four component scores and the composite as data points", () => {
    ...
    const labels = (insight.supportingData?.dataPoints ?? []).map(
      (p) => p.label,
    );
