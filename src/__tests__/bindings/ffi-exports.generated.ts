 ...
 *
 * 28 standalone `#[uniffi::export]` functions plus
 * 278 methods inside `#[uniffi::export] impl` blocks across
 * 15 UniFFI Objects.
 */
 ...
 * Total: 306 exports (28 standalone + 278 methods)
 */
export const FFI_EXPORTS: FfiExportInfo[] = [
  {
    name: 'cancel_fetch_and_store',
    camelName: 'cancelFetchAndStore',
    file: 'ffi.rs',
    line: 59,
    paramCount: 0,
    returnType: 'bool',
    docs: 'Ask the running fetch-and-store to stop. Returns whether there was one. Cooperative and scoped to the run: the loop checks between activities, so the one in flight finishes and lands, and the attach tail still runs over whatever did. The flag is cleared by the reset every run makes, so a cancel cannot outlive the download it was aimed at.',
  },
  {
  ...
  {
    name: 'get_fetch_run_progress',
    camelName: 'getFetchRunProgress',
    file: 'ffi.rs',
    line: 81,
    paramCount: 1,
    returnType: 'DownloadProgressResult',
    docs: "Progress for one fetch run, by the id `start_fetch_and_store` returned. `get_download_progress` answers for the queue head and reports active for any non-empty queue, so a caller whose own run has already finished kept reading active for as long as somebody else's held the slot, and its screen sat on a bar counting someone else's activities. A run that has left the queue reads inactive here, whatever is still downloading.",
  },
  {
    ...
    line: 149,
    paramCount: 1,
    returnType: 'Result<FfiBackupValidation, crate::VeloqError>',
    docs: 'Validate a backup database file without touching the global engine. Opens the file read-only. A record rather than a JSON document, so a field renamed here is a compile error in TypeScript rather than a valid backup reported as invalid at runtime.',
  },
  {
    ...
    paramCount: 2,
    returnType: 'u64',
    docs: "Start one fetch+store run and answer its id. The id is what `take_fetch_and_store_result` reads back with. Three callers start runs, a silent push arriving during a foreground sync is ordinary, and before the id they shared one result slot and took each other's answers.",
  },
  {
    ...
    docs: "Take the result of one fetch+store run. `run` is what `start_fetch_and_store` answered. None means that run has not finished, which is what a caller polls on; it never means another caller's run has finished.",
  },
  {
  ...
  {
    name: 'fetch_and_index_activity',
    camelName: 'fetchAndIndexActivity',
    file: 'ffi.rs',
    line: 1109,
    paramCount: 2,
    returnType: 'Result<crate::FfiIndexActivitySummary, crate::VeloqError>',
    docs: "Fetch one activity's track, store it, and index it against the catalogue. One blocking call, for a caller with no run loop. A push handler in Kotlin or Swift holds an activity id, a budget measured in seconds and no way to poll: `start_fetch_and_store` arms a global slot and hands back a run id, which is the right shape for a screen watching a progress bar and the wrong one here. This returns what happened, or says why nothing did. The three steps each already exist and were only ever composed by the batch path, in a thread that reports through that slot. Composing them here is what lets a native caller hold an id and get a sentence out of it.",
  },
  {
    name: 'new',
    camelName: 'new',
  ...
  },
  {
    ...
    docs: 'Whether the library already holds this activity. An in-memory `contains_key`, so the answer is one boolean rather than the whole id list. The push task asked `get_ids().includes(id)`, which lifted every id string across the bridge to decide one thing.',
    object: 'ActivityManager',
  },
  {
    name: 'get_count',
  ...
  {
    name: 'get_activity_names',
    camelName: 'getActivityNames',
    file: 'objects/activities.rs',
    line: 153,
    paramCount: 1,
    returnType: 'Result<Vec<crate::FfiActivityName>, VeloqError>',
    docs: 'Display names for a batch of activity ids, for a caller that holds ids and has to draw something an athlete recognises. Batched rather than one call per id: a caller drawing a row of chips would otherwise make a blocking FFI hop each, on a screen where a transition is already running. Ids the engine has no name for are absent from the answer rather than carrying an empty string, so the caller can tell "no name" from "a blank name" and fall back to the id.',
    object: 'ActivityManager',
  },
  {
  ...
  {
    name: 'get_preview_track',
    camelName: 'getPreviewTrack',
    file: 'objects/activities.rs',
    line: 283,
    paramCount: 1,
    returnType: 'Result<Option<crate::FfiPreviewTrack>, VeloqError>',
    docs: "One feed card's preview line, from the cached signature. The same thing `get_startup_data` hands the first cards. A card that asks for `get_gps_track` instead pays a full decode of the stored blob and one boxed record per point, for a thumbnail that draws at about a hundred.",
    object: 'ActivityManager',
  },
  {
    name: 'mint_local_id',
  ...
  {
    name: 'init_outcome',
    camelName: 'initOutcome',
    file: 'objects/engine.rs',
    line: 133,
    paramCount: 0,
    returnType: 'crate::objects::init::FfiInitOutcome',
    docs: 'How the last init in this process ended. `is_initialized` says whether the engine is usable, which is what the caller needs to decide what to do next. This says why it is not, which is what the athlete needs to decide what to do about it.',
    object: 'VeloqEngine',
  },
  {
    name: 'is_initialized',
  ...
  {
    name: 'start_clear_routes_and_sections',
    camelName: 'startClearRoutesAndSections',
    file: 'objects/engine.rs',
    line: 185,
    paramCount: 0,
    returnType: 'Result<(), VeloqError>',
    docs: 'Start the route/section wipe on a background thread. Poll `poll_clear_routes_and_sections` for the outcome. The wipe takes the engine write lock like any other writer, so unlike a backup it does not get its own connection. What moves off the calling thread is the wait: a 750-activity library takes 367 ms to wipe, and the caller is the settings toggle, so on the JS thread that is a switch the athlete flipped freezing the app.',
    object: 'VeloqEngine',
  },
  {
    name: 'poll_clear_routes_and_sections',
    camelName: 'pollClearRoutesAndSections',
    file: 'objects/engine.rs',
    line: 205,
    paramCount: 0,
    returnType: 'Result<String, VeloqError>',
    docs: 'Poll the running wipe: "idle" | "running" | "complete". A failed or panicking wipe is an error, and either outcome clears the slot so the next toggle can start one.',
    object: 'VeloqEngine',
  },
  {
    name: 'start_clear_derived',
    camelName: 'startClearDerived',
    file: 'objects/engine.rs',
    line: 232,
    paramCount: 0,
    returnType: 'Result<(), VeloqError>',
    docs: 'Start the clear-cache wipe on a Rust thread. Refuses while one runs.',
    object: 'VeloqEngine',
  },
  {
    name: 'poll_clear_derived',
    camelName: 'pollClearDerived',
    file: 'objects/engine.rs',
    line: 252,
    paramCount: 0,
    returnType: 'Result<DerivedClearPoll, VeloqError>',
    docs: 'Poll the running clear-cache wipe: "idle" | "running" | "complete", and what went once it is complete. Either terminal outcome frees the slot.',
    object: 'VeloqEngine',
  },
  {
    name: 'start_clear_all',
    camelName: 'startClearAll',
    file: 'objects/engine.rs',
    line: 292,
    paramCount: 0,
    returnType: 'Result<(), VeloqError>',
    docs: 'Start the whole-database wipe on a Rust thread. Refuses while one runs. The caller re-opens the engine once this completes, so the poll is what keeps the re-open ordered after the wipe rather than racing it.',
    object: 'VeloqEngine',
  },
  {
    name: 'poll_clear_all',
    camelName: 'pollClearAll',
    file: 'objects/engine.rs',
    line: 306,
    paramCount: 0,
    returnType: 'Result<String, VeloqError>',
    docs: 'Poll the running whole-database wipe: "idle" | "running" | "complete".',
    object: 'VeloqEngine',
  },
  {
  ...
  },
  {
    ...
    paramCount: 0,
    returnType: 'void',
    docs: 'Drop the persistent engine entirely, closing the SQLite connection. The next call to `create()` will re-initialise from scratch. Every running job is asked to stop first. Nothing here waits for them: a cancel is cooperative and read between stages, so a worker already past its last check still finishes, and a detection that finishes after this applies into whatever database is open by then. What the cancel buys is that a job with any of its work left ahead of it stops before reaching the new library, which is the window the restore path opens when it calls this and then `initWithPath` on the restored file.',
    object: 'VeloqEngine',
  },
  {
  ...
  },
  {
    ...
    camelName: 'startBulkExport',
    file: 'objects/engine.rs',
    line: 494,
    paramCount: 2,
    returnType: 'Result<(), VeloqError>',
    docs: "Start a bulk export of every activity with GPS data, in `format`, on a background thread. Poll `poll_bulk_export` for progress and outcome. The file is written from a connection of its own, so neither the JS thread nor the engine's write lock waits for it.",
    object: 'VeloqEngine',
  },
  {
    name: 'poll_bulk_export',
    camelName: 'pollBulkExport',
    file: 'objects/engine.rs',
    line: 516,
    paramCount: 0,
    returnType: 'Result<BulkExportPoll, VeloqError>',
    docs: 'Poll the running export. `state` is "idle" | "running" | "complete", and `exported` against `total` is what a progress bar reads while it runs. A failed export is an error, and either outcome clears the slot so the next export can start.',
    object: 'VeloqEngine',
  },
  {
    name: 'new',
  ...
  },
  {
    ...
    camelName: 'getPowerCurve',
    file: 'objects/fitness.rs',
    line: 126,
    paramCount: 2,
    returnType: 'Result<Option<crate::persistence::bodies::FfiStoredCurve>, VeloqError>',
    docs: 'A stored power curve, or `None` when that sport and window have never been fetched. `None` means "ask for it", not "no data". The fetch time rides along with the body rather than answering a second call, because a curve drawn offline says nothing about its own age and the screens that draw one already make an FFI hop per mount.',
    object: 'FitnessManager',
  },
  {
    name: 'get_pace_curve',
    camelName: 'getPaceCurve',
    file: 'objects/fitness.rs',
    line: 146,
    paramCount: 3,
    returnType: 'Result<Option<crate::persistence::bodies::FfiStoredCurve>, VeloqError>',
    docs: 'A stored pace curve, keyed by sport, window and the gap flag, with the time it was fetched.',
    object: 'FitnessManager',
  },
  {
  ...
  },
  {
    ...
    docs: 'Aggregated totals for one date window: count, duration, distance, TSS.',
    object: 'FitnessManager',
  },
  {
    name: 'get_monthly_stats',
    camelName: 'getMonthlyStats',
    file: 'objects/fitness.rs',
    line: 249,
    paramCount: 2,
    returnType: 'Result<Vec<crate::FfiMonthlyStats>, VeloqError>',
    docs: "A window's totals grouped by calendar month, oldest first. Months with no activity are absent rather than zero.",
    object: 'FitnessManager',
  },
  {
  ...
  {
    name: 'get_wellness_days',
    camelName: 'getWellnessDays',
    file: 'objects/fitness.rs',
    line: 301,
    paramCount: 2,
    returnType: 'Result<Vec<crate::FfiWellnessDay>, VeloqError>',
    docs: 'Stored wellness days over an inclusive date window, oldest first. Typed: every field the wellness and fitness screens render, so nothing parses a body to draw a chart.',
    object: 'FitnessManager',
  },
  {
  ...
  },
  {
    ...
    docs: 'Everything the home-screen widget snapshot is composed from: wellness sparklines, the summary card, and the latest activity with its record flag and GPS track, strided to `max_gps_points`. Replaces the six-call gather in the widget writer. Zero means the whole track.',
    object: 'FitnessManager',
  },
  {
    name: 'new',
  ...
  },
  {
    ...
    paramCount: 3,
    returnType: 'Result<FfiStartOutcome, VeloqError>',
    docs: "Resolve the whole geo component containing (lat, lng) and start the pure preview detect over it. Only the five exposed fields of `config` overlay the engine's live config. The refusals are four opposite answers rather than one `false`. `Held` is a backfill holding detection, a real detect running, or the same component backing off after a failed attempt: all three lift on their own. `Busy` is a run already in flight, which ends. `NotOwed` is no activity covering the point, which no amount of asking changes. `NotReady` is the engine not being open yet, which is early rather than refused. The component, not the point, is the job: two taps a metre apart resolve to one component and so to one key. The five config fields are deliberately **not** in the key. The backoff exists to stop a re-rendering screen asking on every frame, and a slider is what that screen re-renders over, so keying on the config would free the backoff exactly when it is needed.",
    object: 'SectionPreview',
  },
  {
    name: 'poll',
  ...
  {
    name: 'hold_for_network',
    camelName: 'holdForNetwork',
    file: 'objects/recordings.rs',
    line: 117,
    paramCount: 3,
    returnType: 'Result<(), VeloqError>',
    docs: 'The transport failed before the server was reached. The ride keeps its attempts for the same reason a 401 does, and is stamped so the backoff still holds it back.',
    object: 'RecordingManager',
  },
  {
  ...
  {
    name: 'get_map_sections',
    camelName: 'getMapSections',
    file: 'objects/sections.rs',
    line: 146,
    paramCount: 2,
    returnType: 'Result<Vec<crate::FfiMapSection>, VeloqError>',
    docs: '',
    object: 'SectionManager',
  },
  {
  ...
  },
  {
    ...
    paramCount: 2,
    returnType: 'Result<(), VeloqError>',
    docs: 'Mark or unmark a section as a lift. The unmark is durable: `is_lift` is re-derived on every enrichment pass, so the engine records an intent as well as the column. Marking it again takes the intent back off.',
    object: 'SectionManager',
  },
  {
  ...
  },
  {
    ...
    line: 181,
    paramCount: 1,
    returnType: 'crate::objects::start::FfiStartOutcome',
  ...
  },
  {
    ...
    paramCount: 0,
    returnType: 'Result<FfiStartOutcome, VeloqError>',
    docs: 'Start a sync. Returns instantly, naming whether the job started and, if not, whether asking again later would. Work runs on the shared runtime; observe progress via `get_sync_status`.',
    object: 'SyncManager',
  },
  {
    ...
    paramCount: 2,
    returnType: 'Result<FfiStartOutcome, VeloqError>',
    docs: 'Fetch and store one date window of activities. Returns instantly, naming whether the job started and, if not, whether asking again later would. The feed calls this for windows the default sync misses.',
    object: 'SyncManager',
  },
  {
    ...
    line: 1722,
    paramCount: 2,
    returnType: 'FfiStartOutcome',
    docs: 'Fetch and store a power curve for a sport and window. The outcome says why it did not start: `Busy` while the same curve is being fetched, `Held` while a failed one backs off, `NotConfigured` with no credential.',
    object: 'SyncManager',
  },
  {
  ...
  },
  {
    ...
    docs: 'Get total size of heatmap tile cache in bytes. Walks the z/x/y directory tree natively - much faster than JS filesystem calls. Blocking, and linear in cached tiles: 40,061 of them measured 170 ms on the CPH2653, against a 100 ms mount budget. Every mount path uses `start_cache_size` and `poll_cache_size` instead. This stays for the callers that are already off the JS thread and for the tests.',
    object: 'HeatmapManager',
  },
  {
    name: 'start_cache_size',
    camelName: 'startCacheSize',
    file: 'objects/tiles.rs',
    line: 101,
    paramCount: 1,
    returnType: 'Result<(), VeloqError>',
    docs: 'Start the cache-size walk on its own thread. Poll `poll_cache_size` for the figure. A walk already running is not an error: the answer it will give is the answer this caller wants, and three settings mount effects ask for it at once. Starting a second one would walk the same tree twice for one number.',
    object: 'HeatmapManager',
  },
  {
    name: 'poll_cache_size',
    camelName: 'pollCacheSize',
    file: 'objects/tiles.rs',
    line: 126,
    paramCount: 0,
    returnType: 'Result<CacheSizePoll, VeloqError>',
    docs: 'Poll the running walk. `state` is "idle", "running" or "complete", and `bytes` is the last figure walked, whichever of the three it is. The figure outlives the poll that observed it. Only one poll gets the message off the channel, and three mount effects share one walk, so a figure that vanished with the first reader would leave the other two showing nothing. A walk that died reports "complete" against whatever was last known rather than erroring: this is a storage readout, and a screen that cannot show one is not a screen that should refuse to open.',
    object: 'HeatmapManager',
  },
  {
    name: 'poll',
...
export const EXPECTED_TS_FUNCTIONS = new Set<string>([
  ...
  'startClearRoutesAndSections',
  'pollClearRoutesAndSections',
  'startClearDerived',
  'pollClearDerived',
  'startClearAll',
  'pollClearAll',
...
export const RUST_TO_TS_NAME: Record<string, string> = {
  cancel_fetch_and_store: 'cancelFetchAndStore',
  get_download_progress: 'getDownloadProgress',
  get_fetch_run_progress: 'getFetchRunProgress',
  ...
  fetch_and_index_activity: 'fetchAndIndexActivity',
  new: 'new',
  add: 'add',
  get_ids: 'getIds',
  has: 'has',
  ...
  get_activity_names: 'getActivityNames',
  ...
  get_preview_track: 'getPreviewTrack',
  ...
  start_clear_routes_and_sections: 'startClearRoutesAndSections',
  poll_clear_routes_and_sections: 'pollClearRoutesAndSections',
  start_clear_derived: 'startClearDerived',
  poll_clear_derived: 'pollClearDerived',
  start_clear_all: 'startClearAll',
  poll_clear_all: 'pollClearAll',
  ...
  start_bulk_export: 'startBulkExport',
  poll_bulk_export: 'pollBulkExport',
  get_activity_metric_ids: 'getActivityMetricIds',
  get_weekly_summaries: 'getWeeklySummaries',
  get_power_curve: 'getPowerCurve',
  get_pace_curve: 'getPaceCurve',
  ...
  get_monthly_stats: 'getMonthlyStats',
  get_week_load_shape: 'getWeekLoadShape',
  upsert_wellness: 'upsertWellness',
  get_wellness_days: 'getWellnessDays',
  ...
  hold_for_network: 'holdForNetwork',
  ...
  get_map_sections: 'getMapSections',
  ...
  start_cache_size: 'startCacheSize',
  poll_cache_size: 'pollCacheSize',
