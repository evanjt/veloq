...
describe('the sport a section map draws', () => {
  it.each(['GravelRide', 'MountainBikeRide', 'EBikeRide', 'Handcycle', 'Velomobile', 'Treadmill'])(
    'is the section its own, not road cycling: %s',
    (sportType) => {
      expect(styleAskedFor(sportType)).toBe(sportType);
    }
  );

  it.each(['TrackRide', 'Cyclocross'])(
    'is Other for a cycling sport the app vocabulary has no name for: %s',
    (sportType) => {
      // The engine's FTP history knows these two and the app union does not,
      // so they land where every unknown sport lands. Naming them is a
      // vocabulary change, not this one.
      expect(styleAskedFor(sportType)).toBe('Other');
    }
  );
